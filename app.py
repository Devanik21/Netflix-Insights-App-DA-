...
with st.sidebar.expander("📚 In-App Guide"):
    ...
    Explore each tool's expander in the main dashboard for specific functionalities and visualizations.
    """)
    st.markdown("---")
    st.markdown("#### **📄 Data Overview**")
    st.markdown("- **Comprehensive Data Overview:** Initial snapshot of your dataset: first few rows, dimensions (rows, columns), missing value counts, column data types, and a basic statistical summary for all columns.")
    st.markdown("---")
    st.markdown("#### **📊 Core Analytics (Tools 1-10)**")
    st.markdown("- **Tool 1: Content Performance Analytics:** Visualizes IMDb scores vs. viewership (e.g., 'views_millions'), often colored by content type and sized by budget. Shows top-performing titles.")
    st.markdown("- **Tool 2: Genre Trend Analysis:** Tracks the popularity of different genres over release years using line charts, with options for smoothing and selecting top N genres.")
    st.markdown("- **Tool 3: Geographic Content Distribution:** Displays top content-producing countries (primary country) using a bar chart and shows their market share.")
    st.markdown("- **Tool 4: Content Duration Analysis:** Shows histograms for movie durations (in minutes) and TV show durations (in seasons).")
    st.markdown("- **Tool 5: Rating Distribution Analysis:** Presents a pie chart of content ratings (e.g., PG, R, TV-MA) and provides insights into mature vs. family-friendly content percentages.")
    st.markdown("- **Tool 6: Release Year Timeline:** An area chart showing the number of titles released per year, along with a summary of content by decade.")
    st.markdown("- **Tool 7: Budget vs Performance ROI:** Scatter plot analyzing Return on Investment (Views/Budget Ratio) against budget, with options for log scales and opacity. Shows top ROI content.")
    st.markdown("- **Tool 8: Content Correlation Matrix:** Heatmap displaying correlations between all numeric features in the dataset, with a slider to highlight significant correlations.")
    st.markdown("- **Tool 9: Content Gap Analysis:** Identifies potentially underrepresented genres within the top countries (based on a pivot table of country-genre counts).")
    st.markdown("- **Tool 10: Predictive Analytics Dashboard:** Demonstrates simple machine learning models (Linear, Polynomial, SVR) to predict 'Views (Millions)' from 'IMDb Score' and 'Budget'. Includes model evaluation and a prediction interface. *Disclaimer: For illustrative purposes.*")
    st.markdown("---")
    st.markdown("#### **🔬 Advanced Analytics (Tools 11-30)**")
    st.markdown("- **Tool 11: Content Recommendation Engine:** Simple genre-based recommendations, suggesting top-rated titles within a user-selected genre.")
    st.markdown("- **Tool 12: Data Export & Reporting:** Allows downloading the current dataset as CSV or JSON, or an Excel summary report with key metrics and top content lists.")
    st.markdown("- **Tool 13: Director Performance Analysis:** Bar charts showing top directors by the number of titles and average IMDb score (for directors with multiple titles).")
    st.markdown("- **Tool 14: Title Word Cloud:** Generates a word cloud from all content titles to visualize frequently occurring words.")
    st.markdown("- **Tool 15: Content Type Evolution Over Time:** Line chart showing the number of Movies vs. TV Shows released each year.")
    st.markdown("- **Tool 16: Actor/Cast Performance Analysis:** Bar charts for top actors by the number of titles appeared in and by average IMDb score of their titles.")
    st.markdown("- **Tool 17: Genre Deep Dive:** Provides detailed statistics (title count, average IMDb, release trend) for a user-selected genre, and lists its top titles.")
    st.markdown("- **Tool 18: Content Freshness Analysis:** Analyzes content age (current year - release year) with histograms and scatter plots of age vs. IMDb score/views.")
    st.markdown("- **Tool 19: Interactive World Map of Content Production:** A choropleth map visualizing the number of titles produced by each primary country.")
    st.markdown("- **Tool 20: Movie vs. TV Show Deep Comparison:** Side-by-side metrics (total count, avg. IMDb, avg. duration/seasons) and rating distribution pie charts for Movies and TV Shows.")
    st.markdown("- **Tool 21: Release Month/Seasonality Analysis:** Bar chart showing the number of titles added to the platform by month, based on the 'date_added' column.")
    st.markdown("- **Tool 22: Keyword Search in Titles:** Allows users to search for titles containing a specific keyword (case-insensitive).")
    st.markdown("- **Tool 23: Content Rating vs. IMDb Score Analysis:** Bar chart and box plot showing average IMDb scores and distribution for each content rating category.")
    st.markdown("- **Tool 24: Top Director-Actor Collaborations:** Identifies and displays the most frequent director-actor pairings based on co-occurrence in titles.")
    st.markdown("- **Tool 25: IMDb Score Trends Over Release Years:** Line chart showing the average IMDb score of content released each year, with a rolling average trendline.")
    st.markdown("- **Tool 26: Multi-Country Content Profile Comparison:** Compares selected countries based on their content type distribution and average IMDb scores.")
    st.markdown("- **Tool 27: Content Length vs. IMDb Score:** Scatter plots analyzing the relationship between movie duration (minutes) / TV show seasons and IMDb score.")
    st.markdown("- **Tool 28: Genre Co-occurrence Analysis:** Bar chart showing the most frequent pairs of genres listed together for titles.")
    st.markdown("- **Tool 29: Cast Size vs. Performance:** Scatter plots exploring the relationship between the number of cast members and IMDb score/views.")
    st.markdown("- **Tool 30: Content Addition Trend (Yearly):** Bar chart showing the number of titles added to Netflix per year, broken down by content type.")
    st.markdown("---")
    st.markdown("#### **🧠 AI-Powered Tools (Tools 31-35)**")
    st.markdown("*(Requires a Gemini API Key to be entered in the sidebar)*")
    st.markdown("- **Tool 31: AI-Powered General Insights:** Generates high-level strategic insights (e.g., content strategy, market gaps) based on a summary of the dataset using Gemini.")
    st.markdown("- **Tool 32: AI Chat with Dataset:** Allows users to ask natural language questions about the dataset, with answers generated by Gemini based on a provided data summary.")
    st.markdown("- **Tool 33: AI-Generated Content Summaries:** Creates short, engaging Netflix-style summaries for selected titles from the dataset using Gemini.")
    st.markdown("- **Tool 34: AI-Generated Title Suggestions:** Generates creative title ideas with rationales for a new movie/show based on user-provided themes or keywords, using Gemini.")
    st.markdown("- **Tool 35: AI-Driven Sentiment Analysis (Simulated Review):** Analyzes the sentiment (Positive, Negative, Neutral) of a user-provided text (simulating a review) using Gemini.")
    st.markdown("---")
    st.markdown("#### **🔍 Deeper Perspectives (Tools 36-52)**")
    st.markdown("- **Tool 36: Content Lifecycle, Acquisition & Freshness Analysis:** Analyzes content age at addition (acquisition lag), IMDb score vs. lag, performance vs. years on platform, and freshness by type/genre.")
    st.markdown("- **Tool 37: 'Hidden Gems' Detector:** Helps identify content with high IMDb scores but relatively low viewership or budget, using interactive sliders and a scatter plot.")
    st.markdown("- **Tool 38: Genre Popularity vs. Saturation Matrix:** A scatter plot showing genres based on average IMDb score (popularity) vs. number of titles (saturation), with bubble size potentially representing budget.")
    st.markdown("- **Tool 39: N-gram Analysis on Titles:** Identifies and visualizes the most common bi-grams (2-word phrases) and tri-grams (3-word phrases) found in content titles.")
    st.markdown("- **Tool 40: User Persona-Based Recommendations (Simulated):** Provides content recommendations tailored to predefined user personas (e.g., Action Enthusiast, Documentary Lover) based on genre, IMDb, and rating criteria.")
    st.markdown("- **Tool 41: Award Impact Analysis:** Analyzes the average IMDb score and views for titles nominated for 'Best Picture' (if data available) and explores the relationship between the number of awards won and IMDb score.")
    st.markdown("- **Tool 42: Content Language Diversity & Performance:** Shows content distribution by language, average IMDb score by language, average views by language, and content type distribution within top languages.")
    st.markdown("- **Tool 43: Director & Actor Genre Affinity:** For a selected director or actor, visualizes the genres they are most frequently associated with based on their filmography in the dataset.")
    st.markdown("- **Tool 44: Content Attributes & Technical Details vs. Performance:** Analyzes title length (word count) and presence of numbers in titles against IMDb score. If 'aspect_ratio' and 'sound_mix' data exist, it also analyzes their distribution and impact on IMDb score.")
    st.markdown("- **Tool 45: Simulated Franchise/Sequel Analysis:** Attempts to identify potential franchise/sequel titles using title patterns and compares their average IMDb score against likely standalone titles.")
    st.markdown("- **Tool 46: Genre Performance - Movies vs. TV Shows:** For a selected genre, compares the average IMDb score and title count for Movies versus TV Shows within that genre.")
    st.markdown("- **Tool 47: Decade-wise Genre Evolution & Dominance:** An area chart showing how the popularity (number of titles) of top genres has evolved across different decades.")
    st.markdown("- **Tool 48: Budget Efficiency Tiers & ROI Analysis:** Categorizes content into budget tiers (Low, Medium, High) and analyzes ROI distribution and average IMDb score for each tier.")
    st.markdown("- **Tool 49: Emerging Talent Spotlight (Directors/Actors):** Identifies directors and actors with relatively few titles but high average IMDb scores, potentially highlighting 'emerging' talent. Considers ROI if budget/views data available.")
    st.markdown("- **Tool 50: Genre Synergy & Cross-Promotion Opportunities:** Analyzes pairs of genres that frequently co-occur in titles and displays top-performing pairs by average IMDb score, suggesting potential for cross-promotion.")
    st.markdown("- **Tool 51: AI-Driven User Sentiment Analysis (Review Text):** Similar to Tool 35, this uses AI (Gemini, if API key provided) to analyze the sentiment of a user-provided text, simulating review analysis. *This tool is functionally similar to Tool 35 but placed here for broader exploration.*")
    st.markdown("- **Tool 52: Clustering for Content Discovery:** Uses K-Means clustering on selected numeric features (e.g., IMDb score, budget, duration) to identify natural groupings or segments within the content. Results can be visualized using scatter plots (or PCA for >2 features).")
    st.markdown("---")
    st.markdown("#### **🛠️ Utilities (Tool 53)**")
    st.markdown("- **Tool 53: Data Cleaning & Preparation Showcase:** Illustrates common data cleaning steps like handling missing values (with simulation if needed), feature engineering examples (numeric duration, decade creation, genre splitting), and data type conversion.")
    st.markdown("---")
    st.markdown("#### **🎮 Gamified Analytics (Tools 54-55)**")
    st.markdown("- **Tool 54: 'Guess the Views' Challenge:** An interactive game where users guess the viewership of a randomly selected title based on its attributes, earning points for accuracy.")
    st.markdown("- **Tool 55: Netflix Trivia Challenge:** A quiz game with multiple-choice questions generated from the dataset, testing users' knowledge about its content (e.g., top-rated titles, release years, common genres).")
